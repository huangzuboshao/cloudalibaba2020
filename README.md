...
```
### 3.seata
[介绍](http://seata.io/zh-cn/docs/overview/what-is-seata.html)
...
```
提供四大模式:AT(商用-aliyun GTS),GCC,SAGA,XA
默认AT模式：***
    两阶段提交协议的演变：
    一阶段：
...
[介绍](https://github.com/seata/seata)   
```
####一次业务操作需要垮多个数据源或需要垮多个系统进行远程调用,就会产生分布式事务问题####

单体应用->微服务应用(下单(订单服务)-扣库存(仓储服务)-支付(账户服务))
每个服务内部的数据由本地事务保证,但是全局数据一致性没法保证

本地@Transational   ---   全局@GlobalTranstional


seata-server配置:
 file.conf --自定义事务组名称+store模块修改存储为db和db的配置+建库seata建表  + register.conf指明注册中心
